        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="row">
                    <?php
                    $resultsPerPage = 9;
                    $query = "SELECT * FROM product WHERE CategoryID = (SELECT CategoryID FROM category WHERE CategoryName = 'Kids & Babies Clothes')";
                    ...
                    $query = "SELECT * FROM product WHERE CategoryID = (SELECT CategoryID FROM category WHERE CategoryName = 'Kids & Babies Clothes') LIMIT $pageFirstResult, $resultsPerPage";
                    $result = mysqli_query($con, $query);
                ...
                <center>
                    <div class="container">
                        <div class="pagination">
                            <?php
                            if ($page > 1) {
                                echo "<a href='kids.php?page=" . ($page - 1) . "'> Prev </a>";
                            }

                            for ($i = 1; $i <= $numberOfPages; $i++) {
                                echo "<a class='" . ($i == $page ? 'active' : '') . "' href='kids.php?page=" . $i . "'>" . $i . "</a>";
                            }

                            if ($page < $numberOfPages) {
                                echo "<a href='kids.php?page=" . ($page + 1) . "'> Next </a>";
                            }
                            ?>
                        </div>
                        <div class="inline">
...
<script>
    function go2Page(page, numberOfPages) {
        ...
        if (enteredPage >= 1 && enteredPage <= numberOfPages) {
            window.location.href = 'kids.php?page=' + enteredPage;
        } else {
