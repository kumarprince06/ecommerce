 ...
 <div class="breadcrumb-wrap">
            <div class="container-fluid">
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Electronics & Accessories</li>
                </ul>
            </div>
        </div>
        <!-- Breadcrumb End -->
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="row">
                    <?php
                    $resultsPerPage = 9;
                    $query = "SELECT * FROM product WHERE CategoryID = (SELECT CategoryID FROM category WHERE CategoryName = 'Electronics & Accessories')";
                    ...
                    $query = "SELECT * FROM product WHERE CategoryID = (SELECT CategoryID FROM category WHERE CategoryName = 'Electronics & Accessories') LIMIT $pageFirstResult, $resultsPerPage";
                    $result = mysqli_query($con, $query);
                ...
                <center>
                    <div class="container">
                        <div class="pagination">
                            <?php
                            if ($page > 1) {
                                echo "<a href='electronics.php?page=" . ($page - 1) . "'> Prev </a>";
                            }

                            for ($i = 1; $i <= $numberOfPages; $i++) {
                                echo "<a class='" . ($i == $page ? 'active' : '') . "' href='electronics.php?page=" . $i . "'>" . $i . "</a>";
                            }

                            if ($page < $numberOfPages) {
                                echo "<a href='electronics.php?page=" . ($page + 1) . "'> Next </a>";
                            }
                            ?>
                        </div>
...
<script>
    function go2Page(page, numberOfPages) {
        ...
        if (enteredPage >= 1 && enteredPage <= numberOfPages) {
            window.location.href = 'electronics.php?page=' + enteredPage;
        } else {
            window.location.href='electronics.php?page='+numberOfPages;
        }
    }
    // function go2Page(page, numberOfPages) {
    //     var pageInput = document.getElementById("page");
