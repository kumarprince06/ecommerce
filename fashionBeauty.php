 ...
 <div class="breadcrumb-wrap">
            <div class="container-fluid">
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">Fashion & Beauty</li>
                </ul>
            </div>
        </div>
        <!-- Breadcrumb End -->
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="row">
                    <?php
                    $resultsPerPage = 9;
                    $query = "SELECT * FROM product WHERE CategoryID = (SELECT CategoryID FROM category WHERE CategoryName = 'Beauty & Fashion')";
                    ...
                    $query = "SELECT * FROM product WHERE CategoryID = (SELECT CategoryID FROM category WHERE CategoryName = 'Beauty & Fashion') LIMIT $pageFirstResult, $resultsPerPage";
                    $result = mysqli_query($con, $query);
                ...
                <center>
                    <div class="container">
                        <div class="pagination">
                            <?php
                            if ($page > 1) {
                                echo "<a href='fashionBeauty.php?page=" . ($page - 1) . "'> Prev </a>";
                            }

                            for ($i = 1; $i <= $numberOfPages; $i++) {
                                echo "<a class='" . ($i == $page ? 'active' : '') . "' href='fashionBeauty.php?page=" . $i . "'>" . $i . "</a>";
                            }

                            if ($page < $numberOfPages) {
                                echo "<a href='fashionBeauty.php?page=" . ($page + 1) . "'> Next </a>";
                            }
                            ?>
                        </div>
...
<script>
    function go2Page(page, numberOfPages) {
        ...
        if (enteredPage >= 1 && enteredPage <= numberOfPages) {
            window.location.href = 'fashionBeauty.php?page=' + enteredPage;
        } else {
            window.location.href='fashionBeauty.php?page='+numberOfPages;
        }
    }
    // function go2Page(page, numberOfPages) {
    //     var pageInput = document.getElementById("page");
